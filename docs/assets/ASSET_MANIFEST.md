...
| `doyun.normal_suspicious` | `char_doyun_normal_suspicious.png` | 평상복·의심/집중 | 1200×2000 PNG 투명 | 800KB | 필수 | ready | 사용자 생성·직접 제공, exact 모델/작업 ID 미확인; Pillow/NumPy deterministic alpha cleanup | 신규 납품 reference 증빙 미확인·법률 자문 아님 | N1 opening/question, N2 follow-up·identity·chosen | 원본 보존. 2026-08-25 baseline `7a3e963`에서 투명 silhouette에 연결된 purple boundary 4,124px의 alpha만 0으로 정리했다. target RGB·그 밖 pixel 변경 0, source/runtime 동일 바이트·263,531B·SHA-256 `E1CC3909698A3A8A1745E36F165E031F97685671BB83618A4DC8F5517D9913F5`. 사람 장면 QA 대기 |
| `doyun.employee_id_surprised` | `char_doyun_employee_id_surprised.png` | 평상복·사원증 surprise | 1200×2000 PNG 투명 | 800KB | 필수 | ready | 사용자 생성·직접 제공, exact 모델/작업 ID 미확인; Pillow/NumPy deterministic alpha cleanup | 신규 납품 reference 증빙 미확인·법률 자문 아님 | N5 사원증 부상 첫 beat 전용 | 원본 보존. 2026-08-23 magenta fringe cleanup 이력은 유지한다. 2026-08-25 baseline `7a3e963`에서 purple boundary 4,245px의 alpha만 0으로 추가 정리했다. target RGB·그 밖 pixel 변경 0, source/runtime 동일 바이트·291,092B·SHA-256 `16EB7436C74F92AF09FDBD210072DD680936879D789DC67B92746CDA706EF5DA`. HQ-03 `HUMAN_RECHECK` |
...
| `doyun.magical` | `char_doyun_magical.png` | 마법소녀 대표 | 1200×2000 PNG 투명 | 800KB | 필수 | ready | OpenAI base + Codex 내장 `image_gen` 얼굴 reference·deterministic mask 합성, exact 모델 미노출 | internal project lineage·USER ATTESTED·rights PASS 후보 | 전투 fallback | 기존 body·costume·pose·입 모양을 authority로 유지하고 cautious/overwhelmed 얼굴을 공개. source/runtime 동일 바이트·305,801B·SHA-256 `C9D2D15D1FCF0EFC600E63C1DC1B21A6537E7B3418A73BD0CA273963CA674207`; mask 밖 변경 0. HQ-05 `HUMAN_RECHECK`; `approved` 아님 |
| `doyun.magical_defend` | `char_doyun_magical_defend.png` | 마법소녀 방어 | 1200×2000 PNG 투명 | 800KB | 필수 | ready | OpenAI base + Codex 내장 `image_gen` 얼굴 reference·deterministic mask 합성, exact 모델 미노출 | internal project lineage·USER ATTESTED·rights PASS 후보 | battle p1 | 방어 동작의 긴장·입 모양을 유지한 determined/strained 공개 얼굴. source/runtime 동일 바이트·326,121B·SHA-256 `163AE1B01956CFC7C8E4D49F8F7DFA70301E110FCAAE3E9968AB0A047C0185C5`; mask 밖 변경 0. 3개 PC viewport 합성 PASS, HQ-05 `HUMAN_RECHECK`; `approved` 아님 |
| `doyun.magical_attack` | `char_doyun_magical_attack.png` | 마법소녀 공격 | 1200×2000 PNG 투명 | 800KB | 필수 | ready | OpenAI base + Codex 내장 `image_gen` 얼굴 reference·deterministic mask 합성, exact 모델 미노출 | internal project lineage·USER ATTESTED·rights PASS 후보 | battle p2 | 공격 동작의 긴장·입 모양을 유지한 focused 공개 얼굴. source/runtime 동일 바이트·294,540B·SHA-256 `39F9134827B04DCEFD9BB43DAB92DFB99339576B3917A59762414AA0C261F7CD`; mask 밖 변경 0. 3개 PC viewport 합성 PASS, HQ-05 `HUMAN_RECHECK`; `approved` 아님 |
| `doyun.magical_finish` | `char_doyun_magical_finish.png` | 마법소녀 마무리 | 1200×2000 PNG 투명 | 800KB | 필수 | ready | OpenAI base + Codex 내장 `image_gen` 얼굴 reference·deterministic mask 합성, exact 모델 미노출 | internal project lineage·USER ATTESTED·rights PASS 후보 | battle p3 | 마무리 주문의 외침·긴장도를 유지한 forceful 공개 얼굴. source/runtime 동일 바이트·335,301B·SHA-256 `666C7E65B41DC06CE22222BE337FDBB4B25A8C319E77F419BEE395505E5F9658`; mask 밖 변경 0. 3개 PC viewport 합성 PASS, HQ-05 `HUMAN_RECHECK`; `approved` 아님 |
...
### HQ-05 변신 후 공개 얼굴 continuity handoff (2026-08-23 적용)

- 사용자 결정: Option A — CUT01/02에서 처음 공개된 얼굴을 N5~N8 live에서도 유지한다.
- 적용 대상: `doyun.magical`, `doyun.magical_defend`, `doyun.magical_attack`, `doyun.magical_finish`. `doyun.magical_pose`는 identity reference이며 이번 변경에서 수정하지 않았다.
- 각 target의 baseline body·costume·pose·입 모양·긴장도가 authority다. 네 파일을 같은 표정으로 통일하지 않고 기본/방어/공격/마무리의 기존 행동 감정을 각각 유지했다.
- asset pre-edit comparison baseline은 commit `39f4370ab731377b77f3898f48496990961b81cc`다. target별 audited face mask 밖 raw pixel·alpha 변경은 0이고 source/runtime는 동일 바이트다.
- `doyun.magical`은 현재 runtime scene mapping이 없는 fallback이므로 최종 PNG 자체를 검수했다. 나머지 3종은 실제 Chrome 합성에서 1920×1080·1600×900·1366×768 모두 scroll·offscreen·actor overlap 0, console error 0을 확인했다.
- 자동 검증은 기술 계약만 닫는다. 네 공개 얼굴과 실제 합성의 최종 미술 판정은 HQ-05 `HUMAN_RECHECK`이며 사용자 승인 전 `approved`로 승격하지 않는다.
...
- HQ-05 얼굴 continuity는 사용자가 Option A를 선택했고 위 4종 masked face EDIT를 적용했다. [acting/direction audit](V31_ACTING_DIRECTION_AUDIT.md#4-hq-05-transformation-face-continuity--human-decision-required)의 두 option 비교는 결정 이력으로 보존한다.
- 금지: 고통·패배가 아닌 코믹한 당혹감이라는 N5 결과 정서를 절망·심한 괴로움·패배 표정으로 바꾸는 것.
- 이 단계가 편집한 binary는 승인된 `doyun.employee_id_surprised` cleanup, `doyun.magical_pose` head-only EDIT, HQ-05 공개 얼굴 4종뿐이다. Juno action pose와 그 밖 binary는 별도 visual-production 단계 소유다.

...
현재 `assets/runtime/`은 48개 파일, 총 `15,249,374 bytes`(`14.54 MiB`)다. 물리 에셋 30MiB 상한을 통과한다.
...
CDN과 외부 font request는 사용하지 않는다. 상세 출처·hash·notice는 [PROVENANCE.md](PROVENANCE.md)의 `2026-08-15 PC TITLE 로컬 폰트`를 따른다. 현재 runtime은 초반 장면 4종, `doyun.magical_pose` head-only EDIT, `doyun.employee_id_surprised` alpha cleanup 채택 후 `15,249,374B`(14.54MiB)로 30MiB 상한 이하다.

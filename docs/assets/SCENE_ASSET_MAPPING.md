...
| `[확정·M5 폴백 가능]` | `bg_hall_dark` | `bg_hall_dark.webp` | 심야 사무실의 정지한 핵심 팀. 마법이 동료의 시간·인식을 멈춰 위험을 느끼지 못하고, 회색 침식·평가표 잔향은 도윤의 행동 공간에 집중 | N3~N5 도입, 전투 후 수렴 |
...
| `[확정·M5 폴백 가능]` | `bg_transform_space` | `bg_transform_space.webp` | 심야 사무실을 기준으로 사원증과 업무 UI 파편이 빛의 원형 마법진 위로 떠오르는 변신 공간 | N5 영창 결과의 컷 하부 배경 |
| `[확정·M5 폴백 가능]` | `bg_battle_wide` | `bg_battle_wide.webp` | 심야 사무실 전투 stage. 시간·인식이 멈춘 동료를 뒤에 두고 도윤이 보호선을 만드는 p1 도입 | battle p1 |
...
| 우선순위 | 논리 ID | 정확한 파일명 | 제작 내용 | 장면 |
|---|---|---|---|---|
| `[확정·M5 필수]` | `transform.cast` | `cut_transform_01.webp` | 평상복 도윤이 A2 사원증의 은백색 내부 별빛·파란 테두리를 보이며 주문을 말하고, 주노는 기도하는 심야 영창 순간. 텍스트·UI·로고 없음 | N5 주문 성공 직후 1번 |
...
| N2 `n2_juno_followup` | 정체·선택 이유 확인 | `bg_hall_day` → 자연 전이에서만 `bg_hall_dark` omen | 도윤 `normal` + 주노 `neutral`; 냉담만 `normal_tired`. omen은 actor 0 | 계속 `bgm_daily`; N3 진입 뒤 기존 `bgm_crisis` | full facepalm `normal_shy`는 현재 mapping에서 제외. `n2_juno_followup → n3_wraith_choice` 실제 advanced edge에서만 caption `(모니터 속 평가 문장이 한 글자씩 회색으로 번진다.)`를 1회 표시하고 caption 전용 `계속 ›` 입력을 기다린다. 자동 timer·Resume/debug/re-render replay는 0회이며 GameState/save/flags를 쓰지 않는다 |
...
| N6 `n6_first_choice` | 변신 직후 첫 방어/공격 선택 | `bg_hall_dark` | LEFT `gray_wraith.normal` 42vw/780px·80vh/860px 상한 + Doyun-side Juno + RIGHT `doyun.magical_pose` | battle 진입 전 crisis/폴백 | Wraith 확대는 이 presentation context에만 한정한다. actor/UI overlap 0과 dialogue safe zone을 유지하고 이후 Battle scale을 바꾸지 않는다 |
...
- `[확정, 2026-08-22 human direction]` edge actor는 기본적으로 화면 안쪽을 본다. Wraith 2종만 asset audit 후 `SAFE_TO_MIRROR`; Doyun 13종과 Juno 5종은 소품·handedness·의상 비대칭 때문에 `USE_ORIGINAL_ONLY`다. 일괄 `scaleX(-1)`은 금지하며 의도적 외면만 scene 예외다. 상세 판정은 [V31_ACTING_DIRECTION_AUDIT.md](V31_ACTING_DIRECTION_AUDIT.md)를 따른다.
...
- `[구현·사람 재검수]` N2 follow-up의 실제 advanced edge에서만 actor 없는 회색 번짐 omen을 표시하고 caption 전용 `계속 ›` 입력을 기다린다. 자동 timer는 사용하지 않는다. node 진입 자체가 아니라 edge callback에 한정하므로 N3 Resume·debug 직접 진입·ordinary re-render에서는 반복하지 않으며, 직후 N3 대립 구도와 BGM 계약은 변경하지 않는다.

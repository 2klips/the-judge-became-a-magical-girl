...
- 최종 갱신: 2026-08-04
- 이번 갱신: 공개 QA GPT STT 활성화와 무관한 외부 실파일 잔여분 재확인. 신규 논리 ID 추가 없음.
...
| 우선순위 | 추가 작업 | 수량 | 현재 폴백 |
|---:|---|---:|---|
| P0 | 회색 망령 `normal/weakened` 투명 PNG | 2 | 검은 적 presentation |
| P0 | 변신 영창·완료 WebP | 2 | 검은 컷 영역 + 기존 변신 배경/UI |
| P0 | `daily/battle/transform` BGM | 3 | 무음 + Web Audio SFX |
| P0 증빙 | 배경·도윤·주노 생성/권리/공개 사용 근거 | 묶음별 1식 | `ready` 유지, `approved` 승격 금지 |
| P1 | 도윤 변신 기본 포즈 반측면/후면 교체 | 1 | 현재 정면본 임시 사용 |
| P1 | baked NHN/HACKATHON 문구·로고 없는 TITLE | 1 | DEC-037 임시 승인본 |
| P2 | 검은 마법소녀 후속 컷, crisis/ending BGM | 3 | 컷 또는 기존 필수 BGM 폴백 |
| P2 확인 | 과거 17번째 배경의 파일명·용도·실재 여부 | 1식 | 현재 16장만 확정 사용 |

UI·마이크·자막·게이지·SFX 추가 이미지 파일 요청은 없다. 코드 생성 자산으로 유지한다. 위 항목의 장면·파일명·규격은 아래 상세 표가 소유한다.

...
| QD-06 | 대사·응답·엔딩 페이지 직후 진행 버튼 연타 | 2초 전 비활성·진행 0회, 2초 뒤 활성·진행 1회 |
...
| QS-02 | PTT 홀드→발화→release | 같은 버튼이 `듣는 중… 놓으면 전송`→`판정 중…`으로 바뀌고 별도 interim/final 영역 없음 |
...
- [ ] 공개 QA·production 음성 UI에 고정 사용 안내, `STT`, `GPT · 고정`, transcript 원문, 별도 음성 상태 영역이 없고 `누르고 말하기` 계열 PTT 버튼만 표시된다.

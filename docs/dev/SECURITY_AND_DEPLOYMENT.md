...
- 화면·식별: `[확정, DEC-053]` 상단 QA 배너는 표시하지 않는다. 대신 `<html>`의 `data-qa-preview="true"`, `data-qa-commit="7자"`로 배포를 식별하고 검색 엔진에 `noindex,nofollow`를 요청한다. 게임 STT 공급자 선택 UI는 `GPT · 고정`으로 표시한다.

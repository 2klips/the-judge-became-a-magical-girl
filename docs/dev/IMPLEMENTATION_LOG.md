...
- 주노 prompt에 scene-fact direct answer와 완결된 한국어를 요구하고, shared validator가 meta/evasive/context-mismatched/malformed reply를 text와 realtime 양쪽에서 state 적용 전에 거부한다. N2는 `사무실 안쪽`, N3는 망령의 기원·보호 대상·대응 방법을 scene facts로 명시했다. `qa:dialogue-matrix`는 범위 실행 옵션으로 rate limit을 피하면서 N2/N3/N6 16개 발화를 local Worker에 보내며 secret/raw provider body를 다루지 않는다.
